...
Blockly.Lua.scrub_ = function (a, b) {
  ...
  if (!a.outputConnection || !a.outputConnection.targetConnection) {
    ...
    for (var e = 0; e < a.inputList.length; e++) a.inputList[e].type == Blockly.INPUT_VALUE && (d = a.inputList[e].connection.targetBlock()) && (d = Blockly.Lua.allNestedComments(d)) && (c += Blockly.Lua.prefixLines(d, "-- "))
  }
...
Blockly.Lua.lists_split = function (a) {
  ...
  if ("SPLIT" == a) b || (b = "''"), a = Blockly.Lua.provideFunction_("list_string_split", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(input, delim)", "  local t = {}", "  local pos = 1", "  while true do", "    next_delim = string.find(input, delim, pos)", "    if next_delim == nil then", "      table.insert(t, string.sub(input, pos))",
    "      break", "    else", "      table.insert(t, string.sub(input, pos, next_delim-1))", "      pos = next_delim + #delim", "    end", "  end", "  return t", "end"]); else if ("JOIN" == a) b || (b = "{}"), a = "table.concat"; else throw"Unknown mode: " + a;
...
Blockly.Lua.controls_if = function (a) {
  ...
  a.getInput("ELSE") && (e = Blockly.Lua.statementToCode(a, "ELSE"), c += "else\n" + e);
  return c + "end\n"
};
...
Blockly.Lua.logic_operation = function (a) {
  var b = "AND" == a.getFieldValue("OP") ? "and" : "or", c = "and" == b ? Blockly.Lua.ORDER_AND : Blockly.Lua.ORDER_OR,
...
Blockly.Lua.math_single = function (a) {
  ...
  a = "SIN" == b || "COS" == b || "TAN" == b ? Blockly.Lua.valueToCode(a, "NUM", Blockly.Lua.ORDER_MULTIPLICATIVE) || "0" : Blockly.Lua.valueToCode(a, "NUM", Blockly.Lua.ORDER_NONE) || "0";
  switch (b) {
    case "ABS":
      b = "math.abs(" + a + ")";
      break;
...
Blockly.Lua.procedures_defreturn = function (a) {
  ...
  if (Blockly.Lua.STATEMENT_PREFIX) {
    ...
    c = Blockly.Lua.prefixLines(Blockly.Lua.STATEMENT_PREFIX.replace(/%1/g, "'" + d + "'"), Blockly.Lua.INDENT) + c
  }
  Blockly.Lua.INFINITE_LOOP_TRAP && (c = Blockly.Lua.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") + c);
  (d = Blockly.Lua.valueToCode(a, "RETURN", Blockly.Lua.ORDER_NONE) ||
    "") ? d = "  return " + d + "\n" : c || (c = "");
...
Blockly.Lua.text_charAt = function (a) {
  ...
  if ("RANDOM" == b) b = Blockly.Lua.provideFunction_("text_random_letter", ["function " + Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_ + "(str)", "  local index = math.random(string.len(str))", "  return string.sub(str, index, index)", "end"]), a = b + "(" + a + ")"; else {
    if ("FIRST" ==
      b) c = "1"; else if ("LAST" == b) c = "-1"; else if ("FROM_START" != b) if ("FROM_END" == b) c = "-" + c; else throw"Unhandled option (text_charAt).";
...
Blockly.Lua.text_getSubstring = function (a) {
  ...
  if ("FIRST" == c) c = 1; else if ("FROM_START" == c) c = d; else if ("FROM_END" == c) c = "-" + d; else throw"Unhandled option (text_getSubstring)";
  d = a.getFieldValue("WHERE2");
  a = Blockly.Lua.valueToCode(a, "AT2", "FROM_END" == d ? Blockly.Lua.ORDER_UNARY : Blockly.Lua.ORDER_NONE) || "1";
  if ("LAST" ==
    d) a = -1; else if ("FROM_START" != d) if ("FROM_END" == d) a = "-" + a; else throw"Unhandled option (text_getSubstring)";
