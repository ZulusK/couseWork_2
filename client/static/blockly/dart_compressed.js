...
Blockly.Dart.init = function (a) {
  ...
  a = a.getAllVariables();
  if (a.length) {
    for (var c = 0; c < a.length; c++) b[c] = Blockly.Dart.variableDB_.getName(a[c].name, Blockly.Variables.NAME_TYPE);
...
Blockly.Dart.lists_getSublist = function (a) {
  var b = Blockly.Dart.valueToCode(a, "LIST", Blockly.Dart.ORDER_UNARY_POSTFIX) || "[]", c = a.getFieldValue("WHERE1"),
  ...
  } else e = Blockly.Dart.getAdjusted(a, "AT1"), f = Blockly.Dart.getAdjusted(a, "AT2"), a = Blockly.Dart.provideFunction_("lists_get_sublist", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ + "(list, where1, at1, where2, at2) {", "  int getAt(where, at) {",
    "    if (where == 'FROM_END') {", "      at = list.length - 1 - at;", "    } else if (where == 'FIRST') {", "      at = 0;", "    } else if (where == 'LAST') {", "      at = list.length - 1;", "    } else if (where != 'FROM_START') {", "      throw 'Unhandled option (lists_getSublist).';", "    }", "    return at;", "  }", "  at1 = getAt(where1, at1);", "  at2 = getAt(where2, at2) + 1;", "  return list.sublist(at1, at2);", "}"]) + "(" + b + ", '" + c + "', " + e + ", '" + d + "', " + f + ")";
...
Blockly.Dart.text_getSubstring = function (a) {
  ...
  } else e = Blockly.Dart.getAdjusted(a, "AT1"), f = Blockly.Dart.getAdjusted(a, "AT2"), a = Blockly.Dart.provideFunction_("text_get_substring", ["List " + Blockly.Dart.FUNCTION_NAME_PLACEHOLDER_ +
